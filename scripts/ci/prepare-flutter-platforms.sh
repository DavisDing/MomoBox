...
if manifest.exists():
    ...
    if 'android:usesCleartextTraffic="true"' not in text:
        ...
        if replacements != 1:
            raise SystemExit('Unable to locate Android <application> for cleartext NAS access.')
    permissions = (
...
if info_plist.exists():
    ...
    local_network_key = '<key>NSLocalNetworkUsageDescription</key>\n\t<string>用于连接你配置的局域网 NAS 服务。</string>'
    ats_key = '<key>NSAppTransportSecurity</key>\n\t<dict>\n\t\t<key>NSAllowsLocalNetworking</key>\n\t\t<true/>\n\t</dict>'
    missing_usage_descriptions = ''.join(
        f'\t{key}\n' for key, marker in (
            (camera_key, 'NSCameraUsageDescription'),
            (photo_key, 'NSPhotoLibraryUsageDescription'),
            (local_network_key, 'NSLocalNetworkUsageDescription'),
...
# The platform shells are generated in CI, so keep the update installer bridge
# reproducible instead of relying on ignored local Android/iOS files.
python3 - <<'PY'
from pathlib import Path
import re
...
if android_manifest.exists():
    ...
    if permission.strip() not in text:
...
'''
    if 'androidx.core.content.FileProvider' not in text:
        ...
        if replacements != 1:
            raise SystemExit('Unable to locate Android </application> for the update provider.')
...
for gradle in (Path('android/app/build.gradle'), Path('android/app/build.gradle.kts')):
    if gradle.exists():
        ...
        if 'androidx.core:core-ktx' not in text:
            marker = 'dependencies {'
            if marker in text:
                text = text.replace(marker, marker + '\n    implementation("androidx.core:core-ktx:1.15.0")', 1)
            else:
                text += '\ndependencies {\n    implementation("androidx.core:core-ktx:1.15.0")\n}\n'
            gradle.write_text(text)

main_activity = Path('android/app/src/main/kotlin/com/example/momo_box/MainActivity.kt')
if main_activity.exists():
    main_activity.write_text('''package com.example.momo_box
...
class MainActivity : FlutterActivity() {
    private val channelName = "com.example.momo_box/update"
            ...
            when (call.method) {
                ...
                "openUrl" -> {
                    ...
                    if (url.isNullOrBlank()) {
                        result.error("INVALID_URL", "更新页面地址为空", null)
                        return@setMethodCallHandler
                    }
                    try {
...
@objc class AppDelegate: FlutterAppDelegate {
  override func application(
    ...
    if let controller = window?.rootViewController as? FlutterViewController {
      let channel = FlutterMethodChannel(name: "com.example.momo_box/update", binaryMessenger: controller.binaryMessenger)
      channel.setMethodCallHandler { call, result in
        guard call.method == "openUrl",
              let args = call.arguments as? [String: Any],
              let rawUrl = args["url"] as? String,
              let url = URL(string: rawUrl) else {
          result(FlutterError(code: "INVALID_URL", message: "更新页面地址为空", details: nil))
          return
        }
        UIApplication.shared.open(url, options: [:]) { opened in
          opened ? result(nil) : result(FlutterError(code: "OPEN_URL_FAILED", message: "无法打开更新页面", details: nil))
        }
      }
    }
    UNUserNotificationCenter.current().delegate = self
