          ...
          child: ListView(
            ...
            children: [
              ...
              FutureBuilder<List<StockMovement>>(
                future: ref.read(inventoryServiceProvider).loadMovements(item.id),
...
class _BatchTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final state = batch.isDiscarded
        ? ' · 已报废'
        : batch.remainingQuantity == 0
            ? ' · 已耗尽'
            : batch.expiryStatus == ExpiryStatus.expired
                ? ' · 已过期'
                : '';
    return Card(
      child: ListTile(
        title: Text('${batch.batchNo ?? '未填写批次'} · $expiry'),
        subtitle: Text('剩余 ${batch.remainingQuantity}/${batch.initialQuantity} ${item.unit}$state'),
